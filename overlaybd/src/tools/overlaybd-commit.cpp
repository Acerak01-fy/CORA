...
int fastcdc_min_size = 2;   // KB
int fastcdc_avg_size = 8;   // KB  
int fastcdc_max_size = 64;  // KB
...
int main(int argc, char **argv) {
    ...
    // FastCDC options
    app.add_flag("--fastcdc", use_fastcdc, "use FastCDC for content-defined chunking")->default_val(false);
    app.add_option("--cdc_min", fastcdc_min_size, "FastCDC min chunk size in KB [2-64](default 2)")->default_val(2);
    app.add_option("--cdc_avg", fastcdc_avg_size, "FastCDC avg chunk size in KB [4-64](default 8)")->default_val(8);
    app.add_option("--cdc_max", fastcdc_max_size, "FastCDC max chunk size in KB [8-64](default 64)")->default_val(64);
    ...
    if (compress_zfile) {
        ...
        // Configure FastCDC if enabled
        if (use_fastcdc) {
            zfile_args->use_fastcdc = true;
            zfile_args->fastcdc_min_size = fastcdc_min_size * 1024;
            zfile_args->fastcdc_avg_size = fastcdc_avg_size * 1024;
            zfile_args->fastcdc_max_size = fastcdc_max_size * 1024;
            LOG_INFO("FastCDC enabled: min=`KB, avg=`KB, max=`KB", 
                     fastcdc_min_size, fastcdc_avg_size, fastcdc_max_size);
        }
